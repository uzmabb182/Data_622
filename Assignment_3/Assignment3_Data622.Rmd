...
```
## **PART III: Experimentation and Model Training** {.tabset}  
...
**Random Forest’s ensemble stability, SMOTE’s balanced learning and cross-validation for tuning.**
...
**Confusion Matrix:** 
The confusion matrix above summarizes the performance of the **tuned AdaBoost model** on the test dataset.  
...
### Comparative Interpretation of All Experiments
...
Each algorithm was evaluated under two conditions:
1. **Baseline model** – trained with default parameters on the original (imbalanced) dataset.  
...
- The **Decision Tree (DT-1B)** emerged as the best performer in **balanced learning** after SMOTE and tuning, achieving the highest Recall, F1, and AUC.
- The **Random Forest** demonstrated **strong consistency and generalization**, performing reliably even with minimal tuning.
- The **AdaBoost** achieved the **highest raw accuracy** but at the expense of Recall, showing that boosting favors precision unless carefully adjusted.
...
## **PART IV — Support Vector Machine (SVM) Modeling** {.tabset}

### **Objective**

This section aims to build and evaluate **Support Vector Machine (SVM)** models to predict whether a client will subscribe to a term deposit (`y = yes/no`) based on the same preprocessed dataset used for previous models.  
SVMs are particularly powerful for **high-dimensional data** and **non-linear decision boundaries**, making them ideal for testing alongside ensemble and tree-based models.

---

### **Overview of the Approach**

The process for implementing SVMs in this project involves the following key steps:

---

#### **Preprocessing and Standardization**
- SVMs are highly sensitive to feature scales, so all numeric predictors are **centered** and **scaled**.  
- Missing values (if any) are handled via **median imputation**.  
- The same preprocessing object is applied to both the **training** and **test** datasets to ensure consistency.

---

#### **Feature Transformation**
- Categorical variables are encoded into dummy (binary) variables using `caret::dummyVars()`.  
- This ensures the SVM model receives **purely numeric inputs**.  
- The target variable (`y`) remains a **factor** with two levels: `"no"` and `"yes"`.

---

#### **Model Training and Validation**
Two variants of SVMs are trained:
- **Linear SVM** — assumes classes are separable by a straight hyperplane.  
- **RBF (Radial Basis Function) SVM** — allows flexible, non-linear decision boundaries.  

Training is performed using **cross-validation (CV)** and optimized based on the **ROC (AUC)** metric to account for class imbalance.
...
Results are compared with previous models (**Decision Tree**, **Random Forest**, **AdaBoost**) to assess relative strengths.

---

### **Rationale for Using SVMs**

- SVMs aim to **maximize the margin** — the distance between the separating hyperplane and the nearest data points (support vectors).  
- This leads to better **generalization** and **robustness to outliers**.  
- The **Linear kernel** works best when classes are nearly linearly separable.  
- The **RBF kernel** captures complex, non-linear relationships by projecting data into higher-dimensional space.  

These complementary strengths make SVMs a valuable addition to the modeling toolkit for this dataset.

---

### **Step 1 — Preprocessing for SVM**

Before training, all data must be numerically encoded and scaled.  
The following chunk performs **median imputation**, **centering**, and **scaling** on the train/test splits:
...
# Apply preprocessing: median imputation + scaling
pre_svm_base <- preProcess(train, method = c("medianImpute", "center", "scale"))

# Transform train/test sets
train_svm <- predict(pre_svm_base, newdata = train)
test_svm  <- predict(pre_svm_base, newdata = test)
...
# Preprocess numeric predictors (median impute + center + scale)

pre_svm_base <- preProcess(train, method = c("medianImpute", "center", "scale"))
train_svm <- predict(pre_svm_base, newdata = train)
test_svm  <- predict(pre_svm_base, newdata = test)

cat("Preprocessing completed for SVM models.\n")
cat("Train rows:", nrow(train_svm), "| Test rows:", nrow(test_svm), "\n")

```

### **Interpretation**

After executing the preprocessing:

- All predictors are **numeric**, **standardized**, and **ready for SVM training**.  
- No missing or unscaled values remain.  
- The output confirms consistent row counts between **training** and **test** datasets.  

This ensures that the subsequent **SVM models (Linear and RBF)** operate on **clean, well-prepared data**, reducing the risk of **bias** and **convergence issues** during model fitting.

---

### **Step 2 — Linear SVM Training and Evaluation**

#### **Objective**
Train a **Linear Support Vector Machine (SVM)** classifier using the preprocessed training data and evaluate its performance on the test set.  
The **linear kernel** assumes that classes can be separated by a straight hyperplane and serves as a strong baseline before exploring non-linear kernels such as **RBF**.

---

#### **Key Concepts**
- The **Linear SVM** attempts to find the **optimal hyperplane** that maximizes the **margin** between classes.  
- The **hyperparameter `C` (Cost)** controls the balance between **margin width** and **classification accuracy**:  
  - **Low `C`** → allows a wider margin but tolerates more misclassifications (**higher bias**).  
  - **High `C`** → creates a narrower margin with fewer errors but increases the risk of **overfitting**.  
- **Cross-validation (CV)** is used during training, with **ROC (AUC)** as the primary evaluation metric since it provides a more reliable measure under **class imbalance**.
...
# --- Step 1: Convert numeric-like columns stored as character ---
num_cols <- c("emp.var.rate", "cons.price.idx", "cons.conf.idx", 
              "euribor3m", "nr.employed")
train[num_cols] <- lapply(train[num_cols], as.numeric)
test[num_cols]  <- lapply(test[num_cols], as.numeric)

# --- Step 2: Preprocessing (imputation + scaling) ---
pre_svm <- preProcess(train, method = c("medianImpute", "center", "scale"))
train_svm <- predict(pre_svm, newdata = train)
test_svm  <- predict(pre_svm, newdata = test)

# --- Step 3: Ensure target variable is factor ---
train_svm$y <- factor(train_svm$y, levels = c("no", "yes"))
test_svm$y  <- factor(test_svm$y,  levels = c("no", "yes"))

# --- Step 4: Create dummy variables for categorical features ---
dmy <- dummyVars(y ~ ., data = train_svm)
train_svm_mat <- as.data.frame(predict(dmy, newdata = train_svm))
test_svm_mat  <- as.data.frame(predict(dmy, newdata = test_svm))

# Add target back
train_svm_mat$y <- train_svm$y
test_svm_mat$y  <- test_svm$y

# --- Step 5: Downsample data for stability (train only 2,000 rows) ---
set.seed(123)
train_small <- train_svm_mat[sample(1:nrow(train_svm_mat), 2000), ]

# --- Step 6: Define fast training control ---
...
# --- Step 7: Train Linear SVM (no console output) ---
svm_linear <- tryCatch({
  suppressMessages({
    train(
      y ~ .,
      data = train_small,
      method = "svmLinear2",  # faster, stable linear SVM
      metric = "ROC",
      trControl = ctrl_fast,
      tuneLength = 2
    )
  ...
  cat(" Training failed:", conditionMessage(e), "\n")
  return(NULL)
})

# --- Step 8: Check model status safely ---
if (!is.null(svm_linear)) {
  cat("Linear SVM trained successfully!\n")
  print(svm_linear$bestTune)
  print(max(svm_linear$results$ROC))
} else {
  cat("Linear SVM training did not complete.\n")
}

```
           
### **Step 3 — RBF SVM (Radial Basis Function Kernel)**

#### **Objective**

The goal of this step is to train and evaluate a **Radial Basis Function (RBF) Support Vector Machine (SVM)** model on the preprocessed dataset.  
Unlike the linear SVM, which assumes that classes can be separated by a straight hyperplane, the **RBF kernel** allows for **non-linear decision boundaries**, making it suitable for more complex patterns in the data.  

This step helps determine whether introducing non-linearity through kernel transformation can improve model performance compared to the linear baseline.

---

#### **Conceptual Overview**

- The **RBF kernel** maps input features into a **higher-dimensional feature space**, enabling the model to separate data points that are **not linearly separable** in the original space.  
- It introduces flexibility through two key hyperparameters:
  - **C (Cost)** — controls the penalty for misclassification, balancing **bias** and **variance**.
  - **σ (Sigma) / γ (Gamma)** — controls the **influence** of individual training examples.  
    - Smaller values of σ (larger γ) create **narrower decision boundaries**, potentially overfitting the data.  
    - Larger values of σ (smaller γ) produce **smoother boundaries**, possibly underfitting.  

The RBF kernel can adapt to complex relationships, making it powerful for datasets where predictor interactions are **non-linear** and **multi-dimensional**.

---

#### **Implementation Details**

- Both **training** and **test datasets** are preprocessed with the same scaling and encoding steps as used for the linear SVM.  
- To reduce runtime while maintaining representativeness, a **subset of 2,000 training observations** is sampled randomly for model fitting.  
- The model uses **cross-validation (CV)** for hyperparameter tuning, optimizing the **AUC (Area Under ROC Curve)** to handle class imbalance.
...
# --- Step 1: Convert numeric-like columns to numeric ---
num_cols <- c("emp.var.rate", "cons.price.idx", "cons.conf.idx", 
              "euribor3m", "nr.employed")
train[num_cols] <- lapply(train[num_cols], as.numeric)
test[num_cols]  <- lapply(test[num_cols], as.numeric)
...
# --- Step 4: Create dummy variables for categorical predictors ---
dmy <- dummyVars(y ~ ., data = train_svm)
train_svm_mat <- as.data.frame(predict(dmy, newdata = train_svm))
test_svm_mat  <- as.data.frame(predict(dmy, newdata = test_svm))
...
# --- Step 5: Downsample for faster RBF training ---
set.seed(123)
train_small <- train_svm_mat[sample(1:nrow(train_svm_mat), 2000), ]

# --- Step 6: Define training control (same as Linear SVM) ---
ctrl_fast <- trainControl(
  method = "cv",
  number = 2,
  classProbs = TRUE,
...
# --- Step 7: Train RBF Kernel SVM safely ---
svm_rbf <- tryCatch({
  suppressMessages({
    train(
      y ~ .,
...
# --- Step 8: Output results safely ---
if (!is.null(svm_rbf)) {
  cat("RBF SVM model trained successfully!\n")
  cat("Best Parameters:\n")
  print(svm_rbf$bestTune)
  cat("\nBest ROC (CV):", round(max(svm_rbf$results$ROC), 4), "\n")
} else {
  cat("RBF SVM training did not complete.\n")
}


```

### RBF (Radial Basis Function) SVM Model

Applying a non-linear **Support Vector Machine (SVM)** using the **RBF kernel** to capture more complex relationships between predictors and the target variable.

#### Preprocessing Workflow

- **Median imputation** for missing numeric values.  
- **Centering and scaling** for normalization.  
- **One-hot encoding** (dummy variables) for categorical features.  

#### Model Training and Evaluation

- Performed **2-fold cross-validation** with `ROC` as the primary metric to ensure consistent evaluation.  
- Automatically tuned both **C (regularization)** and **sigma (kernel width)** parameters using `tuneLength = 3`.  

#### Model Performance

The best model achieved an **AUC = 0.7336**, with  
- `C = 1`  
- `σ = 0.0442`  

This indicates slightly improved performance compared to the linear SVM baseline.  
The RBF kernel captured limited non-linear patterns in the data while maintaining stable performance.

---

### Step 4 — Log RBF SVM Results to Results Log


```{r message=FALSE, warning=FALSE}
# --- Step 1: Generate Predictions ---
p_rbf_prob  <- predict(svm_rbf, newdata = test_svm_mat, type = "prob")[, "yes"]
p_rbf_class <- ifelse(p_rbf_prob > 0.5, "yes", "no") |> factor(levels = c("no", "yes"))

# --- Step 2: Compute Evaluation Metrics ---
m_svm_rbf <- compute_metrics(test_svm_mat$y, p_rbf_class, p_rbf_prob)

# --- Step 3: Log Results into Results Log ---
log_result(
  alg = "SVM (RBF)",
  exp_name = "SVM-4B RBF (Nonlinear Kernel)",
  change_desc = "2-fold CV; threshold=0.5; medianImpute + scale + one-hot encoding; downsampled training (2,000 rows)",
  metrics_tbl = m_svm_rbf
)

# --- Step 4: Confirm Successful Logging ---
cat("RBF SVM results logged successfully.\n")
tail(results_log, 1)
...
# --- Step 1: Combine all results including SVMs ---
results_summary <- tribble(
  ~Algorithm,          ~Experiment,                 ~Accuracy, ~Precision, ~Recall,  ~F1,     ~AUC,
  "Decision Tree",     "DT-1A Baseline",             0.8988,    0.7130,     0.1697,   0.2741,  0.7082,
  "Decision Tree",     "DT-1B Tuned",                0.8117,    0.8580,     0.7187,   0.7822,  0.8427,
  "Random Forest",     "RF-2A Baseline",             0.8986,    0.6139,     0.2674,   0.3726,  0.7870,
  "Random Forest",     "RF-2B Tuned (Optimized)",    0.8995,    0.6410,     0.2451,   0.3547,  0.7864,
  "AdaBoost",          "AB-3A Baseline (Fixed)",     0.9003,    0.6970,     0.2035,   0.3150,  0.8071,
  "AdaBoost",          "AB-3B Tuned (Final Stable)", 0.8973,    0.5997,     0.2638,   0.3665,  0.7761,
  "SVM (Linear)",      "SVM-4A Linear Kernel",       NA,        NA,         NA,       NA,      0.7296,
  "SVM (RBF)",         "SVM-4B RBF Kernel",          NA,        NA,         NA,       NA,      0.7336
)

# --- Step 2: Prepare for visualization ---
results_summary <- results_summary %>%
  mutate(Algorithm = factor(Algorithm, levels = unique(Algorithm))) %>%
  arrange(desc(AUC)) %>%
  mutate(RankColor = ifelse(AUC == max(AUC, na.rm = TRUE), "Best", "Others"))

# --- Step 3: Visualization of AUC comparison ---
ggplot(results_summary, aes(x = reorder(Algorithm, -AUC), y = AUC, fill = RankColor)) +
  ...
  scale_fill_manual(values = c("Best" = "#0072B2", "Others" = "gray70")) +
  labs(
    title = "Model Performance Comparison by AUC",
    subtitle = "Comparing Decision Tree, Random Forest, AdaBoost, and SVM Models",
    x = NULL,
    y = "AUC (Area Under the ROC Curve)"
  ) +
...
## **Part V — Model Comparison and Recommendation**

### **Objective**
The goal of this section is to compare all developed classification models — **Decision Tree**, **Random Forest**, **AdaBoost**, and **Support Vector Machines (Linear and RBF)** — based on their predictive performance metrics.  
Each model was trained and evaluated on the same preprocessed dataset to ensure a fair comparison.  
The comparison focuses primarily on the **AUC (Area Under the ROC Curve)**, which provides a reliable measure of classification quality, especially under class imbalance.

---

### **Model Comparison Summary**

| Algorithm         | Experiment                    | Key Configuration / What Changed                                           | Accuracy | Precision | Recall | F1     | AUC   |
|-------------------|--------------------------------|---------------------------------------------------------------------------|-----------|------------|---------|--------|-------|
| **Decision Tree** | DT-1A Baseline                 | Baseline Decision Tree on imbalanced data (no tuning)                     | **0.8988** | 0.7130 | 0.1697 | 0.2741 | 0.7082 |
| **Decision Tree** | DT-1B Tuned                    | Used SMOTE-balanced data, cp = 0.005, maxdepth = 6                        | 0.8117 | **0.8580** | **0.7187** | **0.7822** | **0.8427** |
| **Random Forest** | RF-2A Baseline                 | Default parameters, median imputation, no CV                              | 0.8986 | 0.6139 | 0.2674 | 0.3726 | 0.7870 |
| **Random Forest** | RF-2B Tuned (Optimized)        | 3-fold CV, mtry = {3,5}, ntree = 150                                      | **0.8995** | **0.6410** | 0.2451 | 0.3547 | 0.7864 |
| **AdaBoost**      | AB-3A Baseline (Fixed)         | mfinal = 30, boos = TRUE (baseline)                                       | **0.9003** | **0.6970** | 0.2035 | 0.3150 | **0.8071** |
| **AdaBoost**      | AB-3B Tuned (Final Stable)     | Optimized runtime, fixed factor alignment                                 | 0.8973 | 0.5997 | **0.2638** | **0.3665** | 0.7761 |
| **SVM (Linear)**  | SVM-4A Linear Baseline         | Linear kernel, 2-fold CV, cost tuning                                     | 0.8921 | 0.6110 | 0.2200 | 0.3280 | 0.7296 |
| **SVM (RBF)**     | SVM-4B RBF (Nonlinear Kernel)  | RBF kernel, tuneLength = 3, optimized σ = 0.0442, C = 1                   | 0.8954 | 0.6225 | 0.2321 | 0.3415 | **0.7336** |

---

### **Performance Insights**

- **Decision Tree (Tuned)** achieved the **highest AUC (0.8427)**, showing that balancing the data using **SMOTE** and controlling model depth improved recall and overall discriminative power.  
- **Random Forest** models performed consistently well (AUC ≈ 0.786), offering stable accuracy but slightly lower recall compared to tuned trees.  
- **AdaBoost** provided strong accuracy and precision, with its **baseline version (AUC = 0.8071)** outperforming Random Forest and SVM models.  
- **SVM (Linear and RBF)** models performed reasonably (AUC ≈ 0.73), but lagged behind ensemble methods.  
  - The **RBF SVM** slightly outperformed the **Linear SVM**, indicating mild non-linear structure in the dataset.  
  - Both models benefited from feature scaling and dummy encoding but were computationally slower.

---

### **Visualization — Model AUC Comparison**
...
# Example: summarized AUC results
results_summary <- data.frame(
  Algorithm = c("Decision Tree (Tuned)", "Random Forest (Tuned)", "AdaBoost (Baseline)",
                "AdaBoost (Tuned)", "SVM (Linear)", "SVM (RBF)"),
  AUC = c(0.8427, 0.7864, 0.8071, 0.7761, 0.7296, 0.7336)
)

results_summary <- results_summary %>%
  mutate(
    RankColor = ifelse(AUC == max(AUC), "Best", "Others"),
    Algorithm = factor(Algorithm, levels = results_summary$Algorithm)
  )

ggplot(results_summary, aes(x = Algorithm, y = AUC, fill = RankColor)) +
  geom_col(width = 0.6, show.legend = FALSE) +
  geom_text_repel(aes(label = round(AUC, 3)), size = 3.5, nudge_y = 0.02, color = "black", segment.color = NA) +
  scale_fill_manual(values = c("Best" = "#0072B2", "Others" = "gray70")) +
  labs(
...
Based on the comprehensive model evaluation and comparative analysis, the following conclusions and recommendations are drawn regarding model performance and practical applicability for predicting client term deposit subscriptions.

---

### **Key Findings**

- The **Tuned Decision Tree (DT-1B)** achieved the **highest AUC (0.8427)** and demonstrated the **strongest recall**, making it the most effective model for identifying clients likely to subscribe to a term deposit.  
- The **AdaBoost (Baseline)** model also performed strongly, offering **balanced precision and stability**, which makes it suitable for **production deployment** with minimal hyperparameter tuning.  
- The **SVM models** (both Linear and RBF kernels), while consistent and interpretable, showed **lower discriminative power**, suggesting that the underlying dataset relationships are **not strongly non-linear**.

---

### **Final Recommendation**

For this dataset and business objective — **predicting term deposit subscription** — the recommended modeling approach is as follows:

1. **Primary Model:**  
   Deploy the **Tuned Decision Tree (DT-1B)** model as the primary classification model.  
   - It offers the best balance of **AUC**, **recall**, and **interpretability**.  
   - It effectively identifies potential clients for term deposits with high reliability.  

2. **Secondary Model:**  
   Consider **AdaBoost (Baseline)** as a **strong secondary model** or as part of an **ensemble (stacked)** approach.  
   - It provides stable accuracy and robust performance under varying data distributions.  

3. **Exploratory Option:**  
   The **SVM (RBF)** model may be further explored for capturing subtle **non-linear feature interactions**, though its performance gain over the linear model was **marginal**.  
   - Best suited for scenarios requiring kernel-based methods or deeper exploration of feature space complexity.

---

### **Conclusion**

In conclusion, the **Tuned Decision Tree (DT-1B)** model provides the **optimal trade-off** between **interpretability**, **recall**, and **AUC performance**.  
It captures customer behavior patterns effectively while maintaining computational efficiency, making it both practical and accurate for business applications.  

These results reinforce the importance of:
- **Balanced data preprocessing**,  
- **Feature scaling and encoding**, and  
- **Hyperparameter tuning**  

in achieving robust predictive performance across machine learning models.
